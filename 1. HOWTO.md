...
#如果服务端使用`topology subnet`,添加路由使用下面的方式(需要指定网关)
#route 192.168.4.0 255.255.255.0 10.8.0.1
```

`route`语句控制从系统内核到OpenVPN服务器的路由，`iroute`控制从OpenVPN服务器到远程客户端的路由（不是太懂，照做就可以了）。

下一步，请考虑是否允许`client2`所在的子网（192.168.4.0/24）与OpenVPN服务器的其他客户端进行相互通讯。如果允许，在服务器配置文件中添加如下语句（笔者注：可以在`ccd`对应用户名加入，可以更精确控制使用范围，client-to-client只能在服务器配置文件中设置）：
...
经测试，服务端使用`topology subnet`，原文的方式不能使用，可以使用下面示例来分配指定的IP：
...
ifconfig-pool 10.8.0.50 10.8.0.100 255.255.255.0
